...
void print_instr(const InstrBin *instr, int index) {
  printf("Instr[%d]: OP_CODE=%X, ADDR0={prefix:%X, value:%X}, "
         "ADDR1={prefix:%X, value:%X}, ADDR2={prefix:%X, value:%X}\n",
         index, instr->OP_CODE, instr->ADDRESSING_MODE_0.adr_mode_t.prefix,
...
Program par_parse(FILE *file) {
  ...
  for (size_t i = 0; i < program.size; i++) {
    if (allocated - i < 4) {
      allocated += 10;
      program.instrArray =
          realloc(program.instrArray, allocated * sizeof(InstrBin));
    }
    if (i == HEADER_LINE) {
